...
def find_dedup_field(form_name):
    ...
    返回 (componentId, componentName) 或 None(找不到则关闭 upsert)。"""
...
def main():
    ...
    if dedup:
        print(f"[去重键] 使用 {dedup[0]}({dedup[1]}) 作为更新定位键(轻流数据ID)")
    else:
        print("[警告] 未在映射表找到 轻流queId=-17(数据ID)，无法定位更新；待更新记录将被跳过(不更新也不重复创建)")
