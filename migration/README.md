...
```bash
git clone https://github.com/<your-account>/qingflow-to-yida-migrator.git
...
保存后系统自动生成表单配置（`AppKey` + `formUuid` 关联关系），即可开始迁移。凭证与拉取列表的对应接口见「API 文档」小节。
...
4. **写入**：`04` 按差异清单写入宜搭（`batchSave` 新建 / `insertOrUpdate` 更新）。

各脚本支持命令行直跑（推荐先 `--peek` 自检）：
