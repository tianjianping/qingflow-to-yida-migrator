...
ATT_STATS_TTL = 60             # 秒；URL 过期数随时间变化，故设短 TTL
...
def attachment_stats(form_name, refresh=False):
    ...
    if not raw_path.exists():
        return {"hasAttachment": False, "attFields": 0, "totalFiles": 0, "expiredUrls": 0}
    att_que_ids, _, _ = parse_attachment_mapping(form_name)
    if not att_que_ids:
        return {"hasAttachment": False, "attFields": 0, "totalFiles": 0, "expiredUrls": 0}
    try:
        st = raw_path.stat()
        # 缓存 key 纳入宜搭存量与附件台账文件的 mtime：两者更新后自动失效缓存
        yj_mtime = 0
        yida_path_c = DATA / "raw" / f"{form_name}_yida_instances.json"
        if yida_path_c.exists():
            yj_mtime = yida_path_c.stat().st_mtime_ns
        ledger_mtime = 0
        ledger_path_c = DATA / "result" / f"{form_name}_attachment_result.json"
        if ledger_path_c.exists():
            ledger_mtime = ledger_path_c.stat().st_mtime_ns
        cache_key = (st.st_mtime_ns, st.st_size, yj_mtime, ledger_mtime, tuple(att_que_ids))
    except Exception:
        cache_key = None
    if cache_key and not refresh:
        with _att_stats_cache_lock:
            hit = _att_stats_cache.get(form_name)
        if hit and hit[0] == cache_key and hit[1] > time.time():
            return hit[2]
    try:
        raw = json.loads(raw_path.read_text(encoding="utf-8"))
    except Exception:
        return {"hasAttachment": True, "attFields": len(att_que_ids), "totalFiles": 0, "expiredUrls": 0}
    ...
    if cache_key:
        with _att_stats_cache_lock:
            _att_stats_cache[form_name] = (cache_key, time.time() + ATT_STATS_TTL, stats)
    return stats
...
def save_settings(data):
    ...
    if "yidaApps" in data and isinstance(data["yidaApps"], list):
        clean_apps = [{
            ...
            "systemToken": str(a.get("systemToken", "")).strip(),
        ...
        clean_apps = [dict(a, commitDefault=bool(a.get("commitDefault", False)),
                           force=bool(a.get("force", False))) for a in (cred.get("yidaApps", []) or [])]
    if not clean_apps:
        y = cred.get("yida", {}) or {}
    ...
    SETTINGS_PATH.write_text(json.dumps(opt, ensure_ascii=False, indent=2), encoding="utf-8")
    return opt
    ...
    try:
        ...
        for idx, (rec, data_id, total_files, fields) in enumerate(eligible, 1):
            ...
            if att_ledger:
                ...
                if m and m["attachments"] >= total_files:
            ...
            if job.mode == "peek":
                ...
                time.sleep(0.05)
                continue
            field_payloads = []  # [(att_cid, [(url, name, ext), ...])]
            record_ok = True
            for att_que_id, att_vals in fields:
                att_cid = att_cid_map.get(att_que_id)
                payload_items = []
                for v in att_vals:
                    qf_url = v.get("value") or v.get("dataValue") or ""
                    if not qf_url: stats["skipped_bad_url"] += 1; record_ok = False; continue
                    ...
                    if exp_left is not None and exp_left < MIN_EXPIRE_SEC:
                        job.warn(f"[{data_id}] {name} URL已过期 跳过"); stats["skipped_expired"] += 1; record_ok = False; continue
                    ...
                    if not os.path.exists(cache_path) or os.path.getsize(cache_path) == 0:
                        try:
                            ...
                            job.error(f"[{data_id}] 下载失败 {name}: {e}"); stats["errors"] += 1; record_ok = False; continue
                    else:
                        stats["cached"] += 1
                    if storage_cfg.get("upload_token"):
                        ...
                        if digest and digest in content_index:
                            stats["content_hit"] += 1
                            payload_items.append((content_index[digest], name, ext))
                            continue
                        ...
                        try:
                            ...
                            if resp.status_code == 200:
                                ...
                                stats["uploaded"] += 1; payload_items.append((vps_url, name, ext))
                                if digest and vps_url:
                                    content_index[digest] = vps_url
                            else:
                                job.error(f"[{data_id}] 上传失败 HTTP {resp.status_code}: {resp.text[:200]}"); stats["errors"] += 1; record_ok = False
                        except Exception as e:
                            job.error(f"[{data_id}] 上传异常 {name}: {e}"); stats["errors"] += 1; record_ok = False
                    else:
            ...
            if not field_payloads or not record_ok: continue
            if job.mode == "migrate" and job.commit:
                ...
                for att_cid, payload_items in field_payloads:
                    ...
                    # P0-2: formDataJson 不得包含去重键（宜搭限制：同一组件不能既作条件又作更新值）
                    # P0-3: 必须带 searchCondition 才能按数据ID定位目标记录
                    body = {
                    ...
                    try:
                        # P0-1: http_request(url, method=..., headers=..., body=...)，返回已解析的 dict
                        ...
                        if resp and resp.get("success") is False:
                            ...
                            stats["errors"] += 1; write_ok = False
                        else:
                            stats["written"] += 1
                    except Exception as e:
                        job.error(f"[{data_id}] 写入异常: {e}"); stats["errors"] += 1; write_ok = False
                # 台账汇总条目（merged：整条记录的文件数/URL 一览），增量落盘，崩溃不丢进度
                all_urls = [u for _, pl in field_payloads for u, _, _ in pl]
                result_map[str(data_id)] = {
                    "dataID": str(data_id), "attachments": total_files,
                    "urls": all_urls, "success": write_ok, "merged": True,
                }
                if len(result_map) % 10 == 0:
                    _flush_att_result(job.form, result_map)
            if idx % 10 == 0:
                job.info(f"进度: {idx}/{len(eligible)} 条  download={stats['downloaded']} "
                         f"upload={stats['uploaded']} vps命中={stats['vps_hit']} 跳过(已迁移)={stats['skipped_migrated']} 跳过(无宜搭数据)={stats['skipped_no_yida']} written={stats['written']}")
            time.sleep(0.05)
        if result_map:
            _flush_att_result(job.form, result_map)
        save_content_index(local_cache_root, content_index)
        ...
        elif job.mode == "prefetch": job.success(f"预取完成: download={stats['downloaded']} upload={stats['uploaded']} vps命中={stats['vps_hit']} 跳过(已迁移)={stats['skipped_migrated']}", stats)
        else: job.success(f"迁移完成: 写入{stats['written']}条 跳过(已迁移)={stats['skipped_migrated']} 跳过(无宜搭数据)={stats['skipped_no_yida']}", stats)
        job.status = "done"
    except Exception as e:
...
def api_prepare():
    """数据准备：选择表单后一次性串行执行 阶段一(00-02c) + 阶段二(02d) + 阶段三(03)，
    产出最新的 raw / 宜搭存量 / 差异清单 / 转换数据，供后续「更新数据」「迁移附件」使用。
    不含写入(04)，不产生任何宜搭侧修改。"""
    data = request.get_json(silent=True) or {}
    form = data.get("form")
    if not form: return jsonify({"ok": False, "msg": "缺少表单名"}), 400
    ...
    cmds = []
    for sk in ("s1", "s2", "s3"):
        # 准备阶段不强制：02b 保护手工映射，02d 用默认对比（不强制全量更新）
        cmds += build_stage_cmds(sk, form, commit=False, limit=None, force=False)
    jid = start_data_job(form, cmds)
    return jsonify({"ok": True, "jobId": jid,
