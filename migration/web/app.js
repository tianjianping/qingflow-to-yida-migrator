...
(function () {
  ...
  function loadForms() {
    return api("/api/forms").then(function (r) {
      ...
      if (state.selected) {
        ...
        if (still) { state.selected = still.name; renderDetail(); loadFormDetail(still.name); }
      }
    });
  }
  ...
  function loadPreflight(formName) {
    ...
    api("/api/preflight/" + encodeURIComponent(formName)).then(function (r) {
  ...
  function renderPreflight(data) {
    ...
    if (!summary.errors && !summary.warnings && !summary.infos) title = "✅ 迁移预检通过，未发现异常";
    ...
    Object.keys(grouped).forEach(function (cat) {
      html += '<div class="pf-group"><span class="pf-cat">' + esc(cat) + "</span>";
      grouped[cat].forEach(function (c) {
        var lv = c.level === "error" ? "err" : c.level === "warn" ? "warn" : "info";
        html += '<div class="pf-item pf-' + lv + '">' +
  ...
  function testCredentials() {
    ...
    api("/api/credentials/test", { method: "POST" }).then(function (r) {
      var d = r.data || {};
      var rows = "";
